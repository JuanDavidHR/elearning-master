                    ...
                    <div class="row">
                        <div class="col-md-12 table-responsive text-center">
                            <table class="table table-bordered table-striped table-sm mb-0">
                                ...
                                <tbody>
                                    <tr v-for="cuenta in arrayCuenta" :key="cuenta.idCuenta">
                                        <td>
                                            <div class="btn-group" role="group">
                                                <button type="button"  data-toggle="modal" data-target="#proveedor" @click="llenarDatos(1,cuenta.idCuenta)" class="btn btn-info btn-sm" >
                                                    <i style="font-size: 15px;" class="feather icon-eye"></i>
                                                </button> &nbsp;
                                                <!-- <button v-if="cuenta.estado == 'Por Pagar'" type="button" @click="activarEmpresa(cuenta.idCuenta)" class="btn btn-sm btn-success"><i style="font-size: 15px;" class="feather  icon-dollar-sign"></i></button> -->
                                                <button v-if="cuenta.estado == 'Por Pagar'" type="button" class="btn btn-sm btn-success"><i style="font-size: 15px;" class="feather  icon-dollar-sign"></i></button>
                                            </div>
                                        </td>
                                        <td v-text="cuenta.nombre"></td>
                                        <td v-text="cuenta.descripcion"></td>
                                        <td v-text="cuenta.categoria"></td>
                                        <td v-text="cuenta.montoTotal"></td>
                                        <td v-text="cuenta.montoPagado"></td>
                                        <td v-text="cuenta.saldo"></td>
                                        <td>
                                            <span class="badge badge-pill badge-success" v-if="cuenta.vigencia">Habilitado</span>
                        ...
                        <div class="modal-dialog modal-md" role="document">
                            <div class="modal-content mods">
                                <div class="modal-header header-modal">
                                    <h4 class="mb-0">Pagos de Cuenta</h4>
...
export default {
    ...
    data(){
        return {
            ...
            almacen : [],
            tipo : '',
            titleModal : 'Registro de Proveedores',
            nombreProveedor : '',
            dniProveedor : '',
            zonaProveedor : '',
            idProveedor : '',
    ...
    methods: {
        ...
        getMateria(page){
            let me = this;
            var url = '/Cuentas?page='+page;
            axios.get(url).then((response)=>{
                var respuesta = response.data;
                me.pagination= respuesta.pagination;
                me.arrayCuenta = respuesta.cuenta.data;
            }).catch((error)=>{
                console.log(error);
            })
        },
        ...
        llenarDatos(page, idCuenta){
            let me = this;
            var url = '/Cuentas/pago?page='+page+'&idCuenta='+idCuenta;
            axios.get(url).then((response)=>{
                var respuesta = response.data;
                /* me.pagination= respuesta.pagination; */
